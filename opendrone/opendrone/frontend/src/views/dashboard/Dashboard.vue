  ...
  <div class="wrap">
    ...
    <div v-if="auth.isDesigner">
      ...
      <div class="designer-cta">
        <div>
          <h3>Hai un progetto drone da condividere?</h3>
          <p>Caricalo su OpenDrone: verrà esaminato dal team e, se approvato, pubblicato nel marketplace.</p>
        </div>
        <router-link to="/projects/new" class="btn primary">+ Carica nuovo progetto</router-link>
      </div>
    </div>
    ...
    <div v-else>
      <div class="card welcome-card">
        ...
        <p>Esplora il catalogo, trova un progetto drone open source e ordinalo come kit o pre-assemblato.</p>
        <router-link to="/projects" class="btn primary" style="margin-top: 14px; display: inline-flex">
          Esplora il catalogo →
        </router-link>
      </div>

      <!-- Blocco esplicito: upload non disponibile per customer -->
      <div class="no-upload-notice">
        <span class="no-upload-icon">🔒</span>
        <div>
          <div class="no-upload-title">Caricamento progetti non disponibile</div>
          <div class="no-upload-sub">
            Il tuo account è di tipo <strong>customer</strong>.
            Per caricare e vendere progetti registrati come
            <router-link to="/register" class="link-accent">Designer</router-link>.
          </div>
        </div>
      </div>
    </div>
      ...
      <div class="action-list">
        ...
        <router-link v-if="auth.isDesigner" to="/projects/new" class="dash-action">
          ...
          <div class="da-info">
            ...
            <div class="da-sub">STL + BOM + descrizione → revisione admin</div>
          </div>
          <div class="da-arr">→</div>
        </router-link>
...
/* Designer CTA */
.designer-cta {
  background: linear-gradient(135deg, rgba(255,107,53,.06) 0%, rgba(93,255,159,.04) 100%);
...
.designer-cta h3 { font-size: 15px; font-weight: 800; margin-bottom: 4px; }
.designer-cta p  { font-size: 13px; color: var(--muted); }

/* Customer */
.welcome-card { text-align: center; padding: 32px; margin-bottom: 20px; }
...
.no-upload-icon  { font-size: 22px; flex-shrink: 0; }
.no-upload-title { font-size: 13px; font-weight: 700; margin-bottom: 3px; }
.no-upload-sub   { font-family: var(--mono); font-size: 11px; color: var(--muted); line-height: 1.5; }
